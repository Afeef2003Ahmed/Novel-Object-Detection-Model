...
#from CutsomModel_VOC.Utilities import ComputeLoss,ConfusionMatrix,non_max_suppression,xywh2xyxy,process_batch,ap_per_class,LOGGER
from Utilities import ComputeLoss,ConfusionMatrix,non_max_suppression,xywh2xyxy,process_batch,ap_per_class,LOGGER
from Dataset import load_checkpoint,train_loader,test_loader



device = 'cuda:1' if torch.cuda.is_available() else 'cpu'
model_test = Model(num_classes=20,backbone=backbone(),channels_list = [256,512,1024])
...
with torch.no_grad():
  for batch_idx, batch in enumerate(tqdm(train_loader)):
    ...
    temp = outputs
    #print(f"The output of model is {temp[..., 4]}")
    conf_thres = 0.25
    iou_thres = 0.5
    #print('nms')
    ...
    for si, pred in enumerate(eval_outputs):
      ...
      #print(f"The Predictions after NMS are:{pred}")
      nl = len(labels)
...
save_dir = '/raid/cs21resch15003/Afeef_Intern/CutsomModel_VOC/Results'
...
if len(stats) and stats[0].any():
    p, r, ap, f1, ap_class = ap_per_class(*stats, plot=plot_curve, save_dir=save_dir, names=("person", "bird", "cat", "cow", "dog", "horse", "sheep",
            "aeroplane", "bicycle", "boat", "bus", "car", "motorbike", "train",
            "bottle", "chair", "diningtable", "pottedplant", "sofa", "tvmonitor"))
