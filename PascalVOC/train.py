...
from CutsomModel_VOC.Model import Model,backbone

from CutsomModel_VOC.Utilities import ComputeLoss
from CutsomModel_VOC.Dataset import load_checkpoint,train_loader,test_loader,save_checkpoint


device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu:0')

model = Model(num_classes=20,backbone=backbone(),channels_list = [256,512,1024])
...
for epoch in range(num_epochs):
    ...
    if (epoch+1) % 5 == 0:
        ...
        if epoch_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), '/raid/cs21resch15003/Afeef_Intern/best_model.pt')
        else:
